...
3. なければ `setOptions({ key: apiKey, ...GOOGLE_MAPS_OPTIONS })` を1回だけ行う。
...
動的 Provider は 5 ラウンド分に加えて 3 件の予備候補、合計 8 件を生成する。探索枠は Scene ごとに打ち切らずプール全体で共有し、要求 Scene 数 × 40 回（通常は最大 320 回）を上限とする。
...
| パノラマなし地点でクラッシュしない | `ZERO_RESULTS` を棄却し、要求 Scene 数 × 40 回の共有探索枠内で再試行 |
