...
export const initialState: GameState = { phase: "title", roundIndex: 0, scenes: [], candidatePool: [], failedIds: [], currentGuess: null, results: [], error: null };
export type GameAction =
  | { type: "START"; scenes: Scene[]; candidatePool: Scene[] }
...
export function gameReducer(state: GameState, action: GameAction): GameState {
  ...
  if (action.type === "START") return { ...initialState, phase: "round", scenes: action.scenes, candidatePool: action.candidatePool };
  ...
  if (action.type === "IMAGE_FAILED" && state.phase === "round") {
    ...
    if (!candidate) return { ...state, phase: "fatalError", error: "表示できる写真が不足しています。タイトルからやり直してください。", failedIds };
